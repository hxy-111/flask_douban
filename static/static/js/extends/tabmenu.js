...
layui.define(['jquery', 'element'], function(exports) {
    ...
    tabMenu.prototype.tabAdd = function(data, fresh) {
        ...
        if (tab_index === -1) {
            ...
            var content = '<iframes src="' + data.href + '" data-id="' + layID + '" class="jqadmin-iframes"></iframes>';
            var title = '';
    ...
    tabMenu.prototype.effect = function(layID, ischange) {
        ...
        var l = layer.load(1);
        if (ischange) {
            objTab.contentBox.find('iframes[data-id=' + layID + ']').css({ "opacity": "0", "margin-top": "50px" }).delay(200).animate({ opacity: '1', marginTop: "0" }, 500);
            layer.close(l);
        } else {
            objTab.contentBox.find('iframes[data-id=' + layID + ']').css({ "opacity": "0", "margin-top": "50px" }).load(function() {
